function addCopyButtons() {
  document.querySelectorAll('.code-block').forEach(function(block) {
    ...
    btn.onclick = function(e) {
      ...
      var text = '';
      if (pre && window.__practiceCode[pre.id]) {
...
function showToast(message, type) {
  ...
  toast.className = 'success-toast' + (type ? ' ' + type : '');
  toast.innerHTML = '<i class="fas fa-' + (type === 'error' ? 'exclamation-circle' : 'check-circle') + '"></i> ' + message;
  document.body.appendChild(toast);
  setTimeout(function() { toast.remove(); }, 3000);
}

function showLessonEmptyState() {
  var container = document.getElementById('lessonContainer');
  if (container) {
    container.innerHTML = '<div class="empty-state"><i class="fas fa-book-open"></i><h3>Lesson not found</h3><p>This lesson is coming soon. Check back later!</p><button class="btn btn-primary" onclick="showLesson(0)">Start from Chapter 1</button></div>';
  }
}